...
func get_stackable_room(item: Item) -> int:
	if item == null:
		Util.node_error("%s cannot find stackable room for null item", self)
		return 0
...
func fill_existing_stacks(new_item: Item, quantity: int) -> int:
	if quantity <= 0:
		Util.node_error("%s cannot fill existing stacks of %s with quantity of %s", self, new_item, quantity)
		return 0
...
func create_instance(index: int, item: Item, quantity: int, overwrite_occupied := true) -> ItemInstance:
	if item == null:
		Util.node_error("%s cannot create instance at %s with null item", self, index)
		return null
	if quantity <= 0:
		Util.node_error("%s cannot create instance at %s with quantity of %s", self, index, quantity)
		return null
...
func create_new_stacks(item: Item, quantity: int) -> int:
	if item == null:
		Util.node_error("%s cannot create stacks of null item" % self)
		return quantity
...
func give_item(item: Item, quantity: int, to: Inventory) -> int:
	if not is_instance_valid(to):
		Util.node_error("%s cannot give %s to invalid inventory %s", self, item, to)
		return quantity
	if item == null:
		return quantity
	if quantity == 0:
...
func give_everything(to: Inventory) -> void:
	if not is_instance_valid(to):
		Util.node_error("%s cannot give everything to invalid inventory: %s", self, to)
		return
