...
func get_node_saves(scene_root: Node, mode: NodeSave.Mode) -> Array[NodeSave]:
	...
	node_saves.assign(node_properties.filter(func(node_save: NodeSave):
		return node_save.mode == NodeSave.Mode.DYNAMIC and node_save.scene_context == scene_root.scene_file_path
	))
	return node_saves
...
func add_dynamic_nodes(scene_root: Node) -> void:
	if not is_instance_valid(scene_root):
		printerr("%s cannot add dynamic nodes to invalid scene root: %s" % [self, scene_root])
		return
	
	var tree := scene_root.get_tree()
	if not is_instance_valid(tree):
		printerr("%s cannot add dynamic nodes to invalid tree: %s" % [self, tree])
		return
...
func get_parent_node(node_save: NodeSave, spawned_nodes: Dictionary[StringName, Node], non_dynamic_parents: Array[Node]) -> Node:
	if node_save == null:
		printerr("%s cannot get parent node from null node save: %s" % [self, node_save])
		return null
